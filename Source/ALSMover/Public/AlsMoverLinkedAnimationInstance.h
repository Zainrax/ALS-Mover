...
#include "AlsMoverLinkedAnimationInstance.generated.h"

class AAlsMoverCharacter;
class UAlsMoverAnimationInstance;
enum class EAlsHipsDirection : uint8;

/**
 * Linked animation instance for ALS Mover characters
 * This class provides the same interface as UAlsLinkedAnimationInstance but works with Mover components
 * Used for animation layers that need to forward calls to the main animation instance
 */
UCLASS()
class ALSMOVER_API UAlsMoverLinkedAnimationInstance : public UAnimInstance
{
	GENERATED_BODY()

protected:
	UPROPERTY(VisibleAnywhere, Category = "State", Transient)
	TWeakObjectPtr<UAlsMoverAnimationInstance> Parent;

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "State", Transient)
	TObjectPtr<AAlsMoverCharacter> Character;

public:
	UAlsMoverLinkedAnimationInstance();
...
protected:
	...
	// safe to use this function to read variables that change only inside UAlsMoverAnimationInstance::NativeUpdateAnimation()
...
inline UAlsMoverAnimationInstance* UAlsMoverLinkedAnimationInstance::GetParentUnsafe() const
{
	return Parent.Get();
}

inline UAlsMoverAnimationInstance* UAlsMoverLinkedAnimationInstance::GetParent() const
{
	return Parent.Get();
}